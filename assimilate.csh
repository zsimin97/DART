...
set MYSTRING = `grep tau_file_name input.nml`
set MYSTRING = `echo $MYSTRING | sed -e "s#[=,']# #g"`
set TAUFILE = $MYSTING[2]
$LINK gw_tau_0001.nc $TAUFILE
...
set gw_input_file_list = $line[3]

$LIST -1 ${CASE}.cam_[0-9][0-9][0-9][0-9].i.${ATM_DATE_EXT}.nc >! $input_file_list
$LIST -1 gw_tau_[0-9][0-9][0-9][0-9].nc >! $gw_input_file_list
...
set gw_output_file_list = $line[3]
...
if ($cycle == $DATA_ASSIMILATION_CYCLES) then
   ...
   rm /glade/scratch/zsimin/archive/${CASE}/atm/hist/*.preassim.*
   rm /glade/scratch/zsimin/archive/${CASE}/atm/hist/*.i.*
...
for ((ie=1; ie<=$ensemble_size; ie++)); do
  set inst_string = `printf _%04d $ie`
  gw_tau=$(ncdump -v gw_tau -h "gw_tau${inst_string}.nc" | grep "gw_tau =" | awk '{print $3}' | sed 's/;//')
  echo "gw_tau = $gw_tau" >> "user_nl_cam${inst_string}"
done
